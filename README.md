...
- Hosted smoke: `SMOKE_BASE_URL=https://… npm run smoke:hosted` (includes support/privacy/care-updates list + ai + awaiting/ops)
...
- Pilot freeze: `npm run pilot:freeze` (v0.1.9 care receipt close-loop pack)
- Adviser care: `/adviser` radar (`?care=` incl. unacked / awaiting) → 360 Care desk (history + optional ack)
...
- **Release** — git tags `v0.1.0`–`v0.1.9` (MVP → … → care receipts → close-loop); `npm run pilot:freeze`
