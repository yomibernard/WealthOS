...
- Hosted smoke: `SMOKE_BASE_URL=https://… npm run smoke:hosted` (includes support/privacy/care-updates list + ai + awaiting/ops + adviser notifications)
...
- Pilot freeze: `npm run pilot:freeze` (v0.1.22 Ops remind-answer close-loop pack)
...
- **Release** — git tags `v0.1.0`–`v0.1.22` (MVP → … → adviser ops-remind cues → ops remind-answer close-loop); `npm run pilot:freeze`
