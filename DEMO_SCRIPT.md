...
6a. Optional: **Remind linked advisers** on Care handoff (unacked) → Ada **notifications** (**Care handoff** filter) + Care radar **Ops reminded** (`?care=ops_reminded`) / Care desk banner → ack → ops **Recent ops reminds** + **Recent remind answers** (+ admin notify); queues stay open
